...
export class InputPassword {
	...
	init() {
		for (let wrapper of document.querySelectorAll(this.config.selector)) {
...
export class InputFile {
	...
	init() {
		for (let wrapper of document.querySelectorAll(this.config.wrapperSelector)) {
	...
	setInputFile(input, wrapper) {
		let fakeInput = wrapper.querySelector(this.config.fakeSelector);
		let defaultText = fakeInput.innerText;
		let fileName = '';
...
export class Select {
	...
	init() {
		for (let select of document.querySelectorAll(this.config.selectSelector)) {
			let text = select.querySelector(this.config.selectTextSelector);
			text.addEventListener('click', () => this.toggleList(select) );
		}
		
		for (let select of document.querySelectorAll(this.config.selectSelector)) {
			let options = select.querySelectorAll(this.config.selectListItemSelector);
	...
	toggleList(select) {
		...
		for(let item of document.querySelectorAll(this.config.selectSelector)) {
	...
	checkOption(fakeOption, index, selectContainer) {
		let selectText = selectContainer.querySelector(this.config.selectTextSelector);
