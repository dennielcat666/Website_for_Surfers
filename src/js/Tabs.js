...
export default class Tabs {
	...
	init() {
		for (let container of document.querySelectorAll(this.config.container)) {
			container.tabs = new Tab(
				container,
				this.config,
				this.callback
			);
...
export class Tab {
	...
	init() {
		for (let toggle of this.container.querySelectorAll(this.config.toggle)) {
			if (getParent(toggle, this.config.container) != this.container) continue;
			this.toggles.push(toggle);
		}
		for (let tab of this.container.querySelectorAll(this.config.tab)) {
